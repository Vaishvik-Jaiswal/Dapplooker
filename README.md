...
**Docker**
```bash
docker build -t dapplooker .
docker run -d -p 3000:3000 --env-file .env dapplooker
```

## Notes

- CoinGecko API is free but has rate limits
- Groq API offers free tier with fast responses (no cold starts)
- The API gracefully handles AI failures and returns token data even if insight generation fails
- HyperLiquid PnL calculations include realized PnL, unrealized PnL, fees, and funding payments
- Date range for HyperLiquid PnL is limited to 365 days maximum
